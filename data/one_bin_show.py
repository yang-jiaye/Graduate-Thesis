...
def draw_3dframeworks(vis,points,color=[1.,0.,0.]):
...
if __name__ == "__main__":
    ...
    for pred_index in range(len(pred)):
        if pred[pred_index].name == "Car" or pred[pred_index].name == "Pedestrian" or pred[pred_index].name == "Cyclist":
                R = rot_y(pred[pred_index].rotation_y)
                h, w, l = pred[pred_index].dimensions[0], pred[pred_index].dimensions[1], pred[pred_index].dimensions[2]
                x = [l / 2, l / 2, -l / 2, -l / 2, l / 2, l / 2, -l / 2, -l / 2]
                y = [0, 0, 0, 0, -h, -h, -h, -h]
                # y = [h / 2, h / 2, h / 2, h / 2, -h / 2, -h / 2, -h / 2, -h / 2]
                z = [w / 2, -w / 2, -w / 2, w / 2, w / 2, -w / 2, -w / 2, w / 2]
                ...
                # 将该物体移动到相机坐标系下的原点处（涉及到坐标的移动，直接相加就行）
                corner_3d[0, :] += pred[pred_index].location[0]
                corner_3d[1, :] += pred[pred_index].location[1]
                corner_3d[2, :] += pred[pred_index].location[2]
                corner_3d = np.vstack((corner_3d, np.zeros((1, corner_3d.shape[-1]))))
                corner_3d[-1][:] = 1
                ...
                Y = np.dot(inv_Tr, corner_3d)
                draw_3dframeworks(vis, Y, color=[0.,0.,1.])
